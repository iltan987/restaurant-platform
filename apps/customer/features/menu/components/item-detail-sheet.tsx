        ...
        {media.map((m) =>
          m.type === "PHOTO" ? (
            ...
            <img
              ...
              src={m.url}
              alt=""
              className="size-full shrink-0 basis-full snap-center object-cover"
            />
          ) : (
            <video
              key={m.id}
              ...
              controls
              playsInline
              className="size-full shrink-0 basis-full snap-center bg-black object-contain"
            />
          )
        )}
      </div>
 ...
 * later slice). Slides up on open, down on close; Escape and scrim-tap close
 * it, and the page behind is scroll-locked while it's open.
 */
  ...
  return (
    <div className="fixed inset-0 z-50">
      <button
        ...
        onClick={close}
        className={cn(
          "absolute inset-0 bg-[var(--scrim,oklch(0.26_0.03_55/0.46))] backdrop-blur-[3px] transition-opacity duration-300",
      ...
      <div
        ...
        className={cn(
          "absolute inset-x-0 bottom-0 flex max-h-[92%] min-h-[60%] flex-col overflow-hidden rounded-t-[28px] bg-card shadow-[0_-10px_40px_oklch(0.26_0.03_55/0.18)] transition-transform duration-[340ms] ease-[cubic-bezier(0.22,0.8,0.28,1)]",
      ...
      >
        ...
        <div className="menu-scroll flex-1 overflow-y-auto">
          ...
          <div className="px-5 pt-4.5 pb-4">
            <h2 className="font-display text-[27px] leading-[1.08] font-medium tracking-[-0.015em]">
              {item.name}
            </h2>
