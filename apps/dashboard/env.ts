...
/**
 * The dashboard app's environment contract. `createEnv` enforces the
...
export const env = createEnv({
  ...
  client: {
    ...
    // Tenant workspace domain (`<slug>.<root>`).
    NEXT_PUBLIC_ROOT_DOMAIN: z.string().default("localhost:3001"),
    // Customer storefront domain — composes table-QR menu URLs.
    NEXT_PUBLIC_CUSTOMER_ROOT_DOMAIN: z.string(),
  ...
  experimental__runtimeEnv: {
    ...
    NEXT_PUBLIC_CUSTOMER_ROOT_DOMAIN:
      process.env.NEXT_PUBLIC_CUSTOMER_ROOT_DOMAIN,
