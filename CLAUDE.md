...
`specs/004-auth-authorization/plan.md`
<!-- SPECKIT END -->
...
- `@repo/ui` — shared component library (Base UI + shadcn, style `base-nova`), Tailwind v4, `next-themes`, `sonner`. Owns `globals.css`. Consumed via subpath exports like `@repo/ui/components/button`. `cn()` lives in `@repo/ui/lib/utils`. **Base UI uses `render` prop (not `asChild`) to render a primitive as a custom element** — e.g. `<AlertDialogTrigger render={<Button variant="ghost" />}>…</AlertDialogTrigger>`. Never assume Radix/shadcn `asChild` shapes; read the component source or Base UI docs first.
...
- **Pre-commit hooks (husky + lint-staged):** on every `git commit`, staged files are auto-fixed — ESLint (`--fix`) then Prettier (`--write`) — processing **only staged files**. Activated automatically by `pnpm install` (root `prepare: husky` script). Imports are sorted via `eslint-plugin-simple-import-sort` and unused imports removed via `eslint-plugin-unused-imports` (rules live in `@repo/eslint-config/base.js`, so all workspaces + CI `turbo lint` enforce them identically). Group order: side-effect → `node:` → external → `@repo/*` → `@/*` → relative (side-effect imports never reordered). A non-auto-fixable lint **error** blocks the commit; bypass with `git commit --no-verify`. The lint-staged eslint command carries `--flag v10_config_lookup_from_file` (the ESLint 10 default; drop it when upgrading to ESLint 10).
- **TS:** strict, `NodeNext` modules, `noUncheckedIndexedAccess` on. Path alias `@/*` maps to app/src root.
- **NestJS build (SWC/TS6 gotchas):** `baseUrl` in `tsconfig.json` causes SWC to panic — remove it and use path aliases only. The compiled output lands in `dist/src/` (not `dist/`) due to the `rootDir`; `main` in package.json must reflect this.
...
- **Env files & validation:** each app has its own `.env` / `.env.local` (see `.env.example` in each). The **source of truth** for each app's variables — names, types, required/optional — is a validated schema: the api's `apps/api/src/config/env.ts` (Zod, validated fail-fast at boot via `ConfigModule`'s `validate`) and each Next app's `env.ts` (`@t3-oss/env-nextjs` `createEnv`, validated at build, enforcing the server/client split). **Read env through the typed `env` export, never `process.env`** (the lone exception is `next.config.mjs`, which can't import the TS module — it reads `HOSTNAME`/`ALLOWED_DEV_ORIGIN` directly). Adding/removing a var means editing the schema, not hunting scattered reads. The schema-gen CLI and Jest import the api env leniently (`SKIP_ENV_VALIDATION` / `NODE_ENV=test`). Key vars: frontend needs `NEXT_PUBLIC_API_URL`; dashboard needs `NEXT_PUBLIC_ROOT_DOMAIN` + `NEXT_PUBLIC_CUSTOMER_ROOT_DOMAIN`; admin needs `NEXT_PUBLIC_CUSTOMER_URL` (the diner-facing customer storefront — the app a table QR resolves to, and the host shown for a restaurant's menu); api needs `DATABASE_URL`, `ADMIN_URL`, `DASHBOARD_URL`, `CUSTOMER_URL` (the latter three derive CORS origins; `DASHBOARD_URL`/`CUSTOMER_URL` also allow `<slug>.<host>` tenant subdomains — the customer storefront fetches from the browser via TanStack Query, so its origin must be allowed).
