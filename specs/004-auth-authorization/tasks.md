...
- [ ] T006 Generate Better Auth tables into the existing schema: run `pnpm dlx auth@latest generate --config apps/api/src/auth/instances.ts --output packages/db/prisma/schema.prisma --yes`, review the diff, then `pnpm --filter @repo/db db:migrate` and `pnpm --filter @repo/db db:generate`. (Depends on T005.)
